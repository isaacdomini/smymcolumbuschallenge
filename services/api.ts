...
export const createBannerMessage = async (userId: string, messageData: { content: string, type: 'system' | 'user', targetUserIds?: string[], expiresAt?: string, linkUrl?: string }): Promise<void> => {
    if (USE_MOCK_DATA || await isTestUser()) {
        await simulateDelay(300);
        return;
    }
