...
import { BookGuesserData, GameSubmission } from '../../types';
...
const BookGuesserGame: React.FC<BookGuesserGameProps> = ({ gameId, gameData, submission, onComplete, isPreview = false }) => {
  ...
  const handleRevealMore = () => {
    if (revealedVerses < gameData.verses.length) {
  ...
  const handleGuess = (selectedBook: string) => {
    ...
    const isWin = selectedBook.toLowerCase() === gameData.answer.toLowerCase();
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 flex flex-col items-center">
      <h2 className="text-3xl font-bold text-white mb-6 text-center">Book Guesser</h2>
      ...
      <div className="w-full bg-gray-800 rounded-xl p-6 shadow-xl border border-gray-700 mb-8">
        ...
        <div className="space-y-4 mb-6">
          {gameData.verses.slice(0, revealedVerses).map((verse, idx) => (
        ...
        {!isCompleted && revealedVerses < gameData.verses.length && (
          <button
      ...
      {isCompleted && (
        <div className="mt-4 text-center animate-fade-in bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-700 w-full">
          {guess?.toLowerCase() === gameData.answer.toLowerCase() ? (
            <div>
              ...
              <p className="text-gray-300 mb-6 text-lg">You correctly guessed <span className="font-bold text-white">{gameData.answer}</span>.</p>
            </div>
          ) : (
            <div>
              ...
              <p className="text-gray-300 mb-6 text-lg">
                ...
                The correct book was <strong className="text-white text-xl">{gameData.answer}</strong>.
              </p>
            </div>
          )}
          ...
          <div className="bg-gray-900 rounded-lg p-4 mb-6">
            ...
            <p className="text-gray-200">Revealed: <span className="font-bold">{revealedVerses} of {gameData.verses.length}</span> verses</p>
          </div>
